...
function Button_Prompt()
    ...
    Citizen.CreateThread(function()
        local str = _U("traveltomexico")
        TravelToMexico = Citizen.InvokeNative(0x04F97DE45A519419)
        PromptSetControlAction(TravelToMexico, Config.Key)
        str = CreateVarString(10, 'LITERAL_STRING', str)
        PromptSetText(TravelToMexico, str)
        PromptSetEnabled(TravelToMexico, true)
        PromptSetVisible(TravelToMexico, true)
        PromptSetHoldMode(TravelToMexico, true)
        PromptSetGroup(TravelToMexico, buttons_prompt)
        PromptRegisterEnd(TravelToMexico)
    end)
end
...
Citizen.CreateThread(function()
	...
	while true do
        ...
        for i, v in pairs(Config.TravelToMexico) do
            if (Vdist(coords.x, coords.y, coords.z, v.pos.x, v.pos.y, v.pos.z) < 2.0) then
                local TravelToMexico = CreateVarString(10, 'LITERAL_STRING', _U("traveltomexicoprice"))
                PromptSetActiveGroupThisFrame(buttons_prompt, TravelToMexico)
                if IsControlJustReleased(0, Config.Key) then
                    local player = PlayerPedId()
                    local Ped = PlayerPedId()
                    TriggerServerEvent('emotion:ticketbuytomexico')
                    ...
                    if Config.Transitions == true then Citizen.InvokeNative(0x1E5B70E53DB661E5, 0, 0, 0, _U("tomexicotitle"), _U("tomexicosubtitle"), _U("tomexicomessage")) end
...
Citizen.CreateThread(function()
    while true do
        for _, npc in pairs(Config.TravelToMexico) do
